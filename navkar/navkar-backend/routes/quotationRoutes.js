...
router.put("/quotation/prices/:id", async (req, res) => {
  ...
  try {
    ...
    const message =
      updatedCount > 0
        ? `${updatedCount} product(s) updated successfully in quotation and quotation marked as completed`
        : `Quotation marked as completed`;

    return res.status(200).json({ message, quotation });
  } catch (error) {
